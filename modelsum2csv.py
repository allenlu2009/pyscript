...
def parse_shape(shapeLst):
      ...
      dfShape = pd.DataFrame(data, columns = ["Layer", "Channel", "Width", "Height", "Param"]) 
...
def parse_model(modelLst):
      ...
      dfModel = pd.DataFrame(data, columns = ["Layer", "ChI", "ChO", "Kernel", "Stride", "Padding"]) 
...
def merge_shape_model(dfShape, dfModel):
      ...
      for index, row in dfMerge.iterrows():
            if row['Layer'] == 'Conv2d':
                  dfMerge.loc[index, 'MAC'] = row['OFmapX']*row['OFmapY']*row['ChO']*row['ChI']*row['Kernel']**2
                  dfMerge.loc[index, 'ParamW'] = row['Kernel']**2 * row['ChI'] * row['ChO']
                  dfMerge.loc[index, 'ParamIF'] = row['ChI'] * row['IFmapX'] * row['IFmapY']
                  dfMerge.loc[index, 'ParamOF'] = row['ChO'] * row['OFmapX'] * row['OFmapY']
                  dfMerge.loc[index, 'MAC2W'] = dfMerge.loc[index, 'MAC'] / dfMerge.loc[index, 'ParamW']
                  dfMerge.loc[index, 'MAC2IF'] = dfMerge.loc[index, 'MAC'] / dfMerge.loc[index, 'ParamIF']
                  dfMerge.loc[index, 'MAC2OF'] = dfMerge.loc[index, 'MAC'] / dfMerge.loc[index, 'ParamOF']
            elif row['Layer'] == 'Linear':
                  dfMerge.loc[index, 'MAC'] = row['ChO']*row['ChI']
                  dfMerge.loc[index, 'ParamW'] = (row['ChI']+1) * row['ChO'] # add bias 
      ...
      logger.debug("Param match? : %s" %(dfMerge['Param'].equals(dfMerge['ParamW'])))
